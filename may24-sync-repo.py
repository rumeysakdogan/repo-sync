...
def get_default_branch(repo_name: str) -> str:
    ...
    # org = g.get_organization(ORG_NAME) # retrieve repo from org
...
def main():
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(create_ado_repository, DESTINATION_ORG, DESTINATION_PROJECT, repo.name) for repo in new_repos]
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(synchronize_and_push, repo.name) for repo in existing_repos]
    ...
    logging.info(f'Synchronization time: {exec_time.tm_hour}h {exec_time.tm_min}m {exec_time.tm_sec}s')
