...
def list_ado_repositories(org_name: str, project_name: str) -> list:
    destination_repos = []
    cmd = f'az repos list --org https://dev.azure.com/{org_name} --project {project_name}'
...
def clone_and_push(repo_name: str) -> tuple:
    try:
        ...
        logging.debug(f"Clone URL: {clone_url}")
        ...
        logging.debug(f"Repo cloned successfully")

        # List files in repo_path
        files = os.listdir(repo_path)
        logging.debug(f"Files in repo_path: {files}")

        # Remove history and initialize new repo
        shutil.rmtree(os.path.join(repo_path, ".git"))
        Repo.init(repo_path)
        logging.debug(f"History removed and new repo initialized")
        ...
        logging.debug(f"Initial commit made")
        ...
        logging.debug(f"Remote URL: {remote_url}")
        ...
        logging.debug(f"Changes pushed to destination")
...
def main():
    ...
    for repo in new_repos:
        ...
        if result is not None:
            ...
            if error:
                print(f"Error: Task failed with exception: {error} for repository: {repo_name}")
